  ...
  <div>
    ...
    >
      <template v-slot:cell(actions)="row">
        <b-button @click="getDetails(row.item)">{{ row.item.actions.text }}</b-button>
      </template>
    </b-table>
    <div v-for="legend in legends" :key="legend.description" class="legend">
      <div>
        <div class="boxes" :class="'bg-' + legend.color"></div>
        {{ legend.description }}
      </div>
    </div>
  ...
  data() {
    return {
      legends: [
        {
          description:
            "Indicates that there are no new cases and all active cases have recovered",
          color: "success"
        },
        {
